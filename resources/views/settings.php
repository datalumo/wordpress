...
<div class="wrap datalumo-settings">
    ...
    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
        ...
        <?php if ($datalumo_tab === 'connection') : ?>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" id="datalumo-organisation-id" class="regular-text code"
                               value="<?php echo esc_attr($datalumo_organisation['id'] ?? ''); ?>" />
                        ...
                        <span id="datalumo-connect-result" class="datalumo-inline-result">
                            <?php if (Options::isConnected()) : ?>
                                ...
                                printf(
                                    ...
                                    esc_html($datalumo_organisation['name'] ?? $datalumo_organisation['id'] ?? ''),
                                    count($datalumo_sources),
                                );
                                ?>
                            <?php endif; ?>
                        </span>
            ...
            <?php endif; ?>
        <?php elseif ($datalumo_tab === 'content-sync') : ?>
            <?php if (! Options::isConnected()) : ?>
            ...
            <?php else : ?>
                ...
                <div id="datalumo-syncs">
                    <?php foreach ($datalumo_syncs === [] ? [[]] : $datalumo_syncs as $datalumo_sync_index => $datalumo_sync) : ?>
                        <fieldset class="datalumo-sync-row" data-index="<?php echo esc_attr((string) $datalumo_sync_index); ?>">
        ...
        <?php elseif ($datalumo_tab === 'chatbot') : ?>
            ...
            <?php if (WooCommerce::isActive()) : ?>
                <p class="description"><?php esc_html_e('WooCommerce is active. In Datalumo, add an Act on the page action with the event name add_to_cart and a product_id field (the WordPress product id). After the visitor confirms in chat, this plugin adds the item to their cart.', 'datalumo'); ?></p>
            <?php endif; ?>
            <?php submit_button(); ?>
